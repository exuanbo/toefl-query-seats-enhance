import { firstKeyOf, isMunicipality } from '../utils'
import { QueryData, SeatDetail } from '../data'
...
export const Table = ({
  ...
  <thead>
    <tr style="background-color:#993333;">
      <th colspan="4">
        ...
        ><span style="color:#fff;float:right;"
          >考试时间：${firstKeyOf(testSeats).split('|')[0]}<span
    ...
    <tr>
      <th style=${styleMap(stylesMiddle)} width="20%">城市</th>
      <th style=${styleMap(stylesMiddle)}>考点</th>
      <th style="text-align:center;" width="20%">费用<br />(RMB￥)</th>
      <th style=${styleMap(stylesMiddle)} width="10%">考位</th>
    </tr>
  </thead>
  <tbody>
    ${testSeats[firstKeyOf(testSeats)].map(
      (seat: SeatDetail): TemplateResult => html`${rowTpl(seat)}`
    )}
  </tbody>
`

const rowTpl = (seat: SeatDetail): TemplateResult => html`
  <tr>
    ...
    <td style=${styleMap(stylesMiddle)}>
      ...
      ${seat.lateRegFlag === 'Y' ? html`<br />(已包含逾期费附加费)` : nothing}
    </td>
    <td style=${styleMap(stylesMiddle)}>
      ${seat.seatStatus === -1
        ? '已截止'
...
const formatCurrency = (value: number): string => 'RMB￥' + value.toFixed(2)
