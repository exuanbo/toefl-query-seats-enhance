    ...
    <table class="table table-bordered" style="margin-top:12px;font-size:16px;">
      <thead>
        <tr style="background-color:#993333;">
          <th colspan="4"><span style="color:#fff;">考试日期：${data.testDate}</span><span style="margin-left:.5em;color:#fff;"><i class="fa fa-calendar-check-o" aria-hidden="true"></i></span><span style="color:#fff;float:right;">考试时间：${helper.firstKeyOf(data.testSeats).split('|')[0]}<span style="padding-left:30px;">最晚到达时间：${helper.firstKeyOf(data.testSeats).split('|')[2]}</span></span></th>
        </tr>
        <tr>
      ...
      <tbody>
        ${data.testSeats[helper.firstKeyOf(data.testSeats)].map(seat => html`${rowTpl(seat)}`)}
      </tbody>
    </table>
  `
